...
def index(request):
    ...
    rad_record = Radacct.objects.using('freeRadius').values('username').filter(framedipaddress=user_ip,
                                                                               acctstoptime__isnull=True).first()
    if not rad_record:
        return Response("ops! no user for this ip!")

    result['username'] = rad_record['username']
    # What is user groupname?!
    user_groupname = Radusergroup.objects.using('freeRadius').filter(username=result['username']).first()
    if user_groupname:
    ...
    # what is user speed
    user_speeds = Radgroupreply.objects.using('freeRadius').filter(groupname__contains=result['groupname'],
                                                                   attribute="Mikrotik-Rate-Limit").all()
    ...
    package = Radpackages.objects.using('freeRadius').filter(groupname=result['groupname']).first()
    if package:
        result['package'] = {
    ...
    # user usage
    raddaily_record = Raddaily.objects.using('freeRadius').filter(username=result['username'],
                                                        createddate=datetime.datetime.today().date()).first()
    raddaily_record_monthly = Raddaily.objects.using('freeRadius').filter(username=result['username'],
                                                                          createddate__lte=datetime.datetime.today().date(),
                                                                          createddate__gt= datetime.datetime.today().date() - datetime.timedelta(days=30))
    raddaily_record_weekly = raddaily_record_monthly.filter(createddate__lte=datetime.datetime.today().date(),
    ...
    for session in \
            Radacct.objects.using('freeRadius').filter(username=result['username'], acctstoptime__isnull=True).all():
...
def user_usage(request):
    ...
    if not username:
        rad_record = Radacct.objects.using('freeRadius').values('username').filter(framedipaddress=ip,
                                                                                   acctstoptime__isnull=True).first()
        if not rad_record:
            return Response("ops! no user for this ip!")
    ...
    if result['username'] == 'guest':
        ...
        result['sessions'] = []
        for session in \
                Radacct.objects.using('freeRadius').filter(username=result['username'],
                                                           framedipaddress=ip,
                                                           acctstoptime__isnull=True).all():
            result['sessions'].append({
                'framedipaddress': session.framedipaddress,
                'acctstarttime': session.acctstarttime,
                'acctsessionid': session.acctsessionid,
    ...
    # what is user speed
    user_speeds = Radgroupreply.objects.using('freeRadius') \
    ...
    # What is user packages
    package = Radpackages.objects.using('freeRadius').filter(groupname__contains=result['groupname'].split('-')[0]).first()
    if package:
        result['package'] = {
            'daily': package.daily_volume,
            'weekly': package.weekly_volume,
    ...
    dailyusagerecords = Raddaily.objects.using('freeRadius') \
        .filter(username=result["username"],
                createddate__lte=datetime.datetime.today().date(),
                createddate__gt=datetime.datetime.today().date() - datetime.timedelta(days=30)) \
        .order_by('-createddate').all()

    result['usagehistory'] = []
    for record in dailyusagerecords:
        result['usagehistory'].append({
            'created_date': record.createddate,
            'usage': record.usagediscount
        })
    return Response(result)
...
def logout(request):
    ...
    logging.info("logout for user {0} , ip {1} and acctuniqueid {2} ".format(username, ip, acctuniqueid))

    rad_record = Radacct.objects.using('freeRadius').filter(username=username,
                                                            framedipaddress=ip,
                                                            acctuniqueid=acctuniqueid).first()
    if rad_record:
        if rad_record.acctstoptime:
            # TODO: it's mean that there is some problem
            logging.info("logout for user {0} and ip {1}: user have stoptime.Why?".format(username, ip))
            return Response("OK")
        elif disconnect(username, ip):
            return Response("OK")
        else:
            # let's disconnect it again if fail
            logging.info("logout for user {0} and ip {1}: Trying again!".format(username, ip))
        ...
        logging.info("logout for user {0} and ip {1}: user not found!".format(username, ip))
        return Response("OK")
